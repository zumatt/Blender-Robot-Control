# Blender - Robot Control

> [!IMPORTANT]
> This plugin, based on the [Parametric Robot Control - Blender Geometry Nodes](https://github.com/jbraumann/PRC.Integrations/tree/main/Blender%20Geometry%20Nodes), is made for research purposes and tested with a single machine, specifically a Kuka KR 6. For that reason, is provided “as is” without any warranties or guarantees. The creators and developers of this software do not accept any liability for any damages or losses arising from its use.

## How to install

### 1. Download the add-on

1. Go to the [Releases](https://github.com/zumatt/Blender-Robot-Control/releases) page of this repository.
2. Find the latest release and download the `prc_blender.zip` file (or the file labeled as the plugin release).
3. Save it somewhere you can find it (e.g., Downloads folder).

> **Do not extract the zip manually.** Blender will handle the installation directly from the `.zip` file.

### 2. Install in Blender

1. Open **Blender**.
2. Go to **Edit** → **Preferences** (or **Blender** → **Preferences** on macOS).
3. In the left sidebar, click **Add-ons**.
4. Click the **Install...** button (top-right area of the Add-ons panel).
5. A file browser opens. Navigate to where you saved `prc_blender.zip` and select it.
6. Click **Install Add-on** to complete the installation.

> Blender automatically extracts the zip and places the add-on in the correct location.

### 3. Enable the add-on

1. After installation, the Add-ons list refreshes. Search for **"PRC"** or **"Parametric Robot Control"** to find the newly installed add-on.
2. Click the **checkbox** next to the add-on name to enable it.
3. You should see a green checkmark and a **"Saved to"** confirmation message.

### 4. Verify installation

1. Open or create a Blender project.
2. Open the **3D Viewport** and press **N** to toggle the side panel (or find the **N-panel** on the right).
3. You should see the **Parametric Robot Control** panel with a gear icon and a **Robot Setup** box.

If you see the panel, the add-on is ready. Proceed to the **Connect to the Robot** section below.

### Troubleshooting

**"Add-on failed to load"**
- Confirm Blender version is **5.0.0 or later** (see Prerequisites).
- Check the **System Console** (Windows: `Window → Toggle System Console`; macOS/Linux: run Blender from terminal) for error messages.
- Try removing the add-on (`Edit → Preferences → Add-ons`, search, click the X button) and reinstalling.

**"No .NET runtime found" error (Windows/Linux/macOS)**
- The embedded server requires .NET 8.0 or later.
- Install from [microsoft.com/net](https://dotnet.microsoft.com/download) or your OS package manager.
- Restart Blender and try **Start Embedded Server** again.

**Panel does not appear**
- Ensure the add-on is checked (enabled) in the Add-ons list.
- Close and reopen the 3D Viewport side panel (press **N**).
...
- **Blender 5.2.0+** is required only for **Insert Code**. The **Set Output**, **Wait**, **Wait For Input**, **Brake**, **Interrupt Declaration**, **Interrupt Control**, and **Function Definition** nodes work on Blender 5.0 / 5.1 too. Insert Code depends on string-typed Geometry Nodes attributes that landed in 5.2, so on 5.0 / 5.1 only that single entry is hidden from the Add menu.
...
- A bundled `PRC.Server` folder inside the add-on directory (or any PRC server already listening on `https://127.0.0.1:5001`).
- For framework-dependent server bundles (`PRC.Server.dll`), install the matching .NET runtime on your OS.
...
## Agent folder (for contributors)

If you use AI agents to modify this repository, see [agent/README.md](agent/README.md).

- The `agent` folder is dedicated to agent instructions and should stay minimal.
- The canonical machine-readable instructions are in `agent/LLM_Checklist.yaml`.
- When repository logic changes, update the YAML so agent behavior remains aligned.
...
2. *(Optional)* Pick **Custom Header** (a Blender Text datablock). Its content is injected at program export/header level.
3. Pick a **Driver** (e.g. `KUKA KSS (KRL)`). The Robot dropdown auto-filters to that brand.
4. Pick a **Robot** from the filtered list.
5. *(Optional)* In the **Tool** box, set the TCP — Position (mm) and Rotation. The rotation fields adapt to the driver convention (ZYX degrees for KUKA, RPY for NEURA, axis-angle for UR, quaternion for ABB). The Tool ID determines which dictionary slot the TCP lives at.
6. Click **Setup Robot**.

On Setup, the add-on auto-starts the embedded server if port `5001` is not already in use. You can also start/stop it manually with **Start Embedded Server** / **Stop Embedded Server** in Robot Setup. **Open Server** opens the PRC web UI at `https://127.0.0.1:5001`.
...
- **Generate Geometry Node Groups** is non-destructive: existing PRC node-group instances and links in your current Geometry Nodes tree are kept; only missing groups are created.
- **Load Task from Geometry Nodes** — reads the evaluated `PRC_Program` carrier mesh and sends it as task payload.
...
| **Motion Groups** | `PRC PTP Motion Group`, `PRC Cartesian Motion Group`, `PRC Action Group` |
...
| **Actions** | `PRC Set Output`, `PRC Wait`, `PRC Wait For Input`, `PRC Brake`, `PRC Interrupt Declaration`, `PRC Interrupt Control`, `PRC Function Definition`, `PRC Insert Code` *(5.2+ only)* |
| **Task** | `PRC Task` |

The dedicated action nodes are convenience wrappers. `PRC Insert Code` remains available for any custom line that does not need its own specialized node.
...
### Set Output
Inputs: `Port` (Int), `State` (Bool).

Emits inline code as:

`$OUT[PORT] = TRUE/FALSE`

Where `PORT` is the selected output port and `TRUE/FALSE` comes from the `State` toggle.
...
Where `<seconds>` is the numeric value you provide.
...
Where `PORT` is the input port number.
...
If `Stop 1` is enabled, the node emits `BRAKE F` (Stop 1). If `Stop 1` is disabled, the node emits `BRAKE` (Stop 2).

### Interrupt Declaration
Inputs: `Priority` (Int), `Is Global` (Bool), `Port` (Int), `Condition` (Bool), `Subprogram ID` (Int), `Subprogram` (String).

Declares a KRL interrupt handler with the syntax:

`[GLOBAL] INTERRUPT DECL Prio WHEN $IN[Port]==Condition DO Subprogram ()`

Notes:
- Priority uses KUKA user-valid ranges: `1`, `2`, `4..39`, `81..128`.
- On Blender 5.0 / 5.1, string attributes are limited; `Subprogram ID` is used as fallback (`UP<ID>`).
- Declaration only defines the interrupt. Activate/deactivate it with **Interrupt Control**.

### Interrupt Control
Inputs: `Number` (Int), `Enable` (Bool).

Emits inline code as:

`INTERRUPT ON Number` or `INTERRUPT OFF Number`

### Function Definition
Inputs: `Geometry`, `ID` (Int).

Tags connected action geometry as subprogram body `UP<ID>`. During **Export Program**, these tagged action lines are appended after the main program `END` as:
...
Use this for interrupt handlers and reusable user procedures.

### Insert Code
Inputs: `Code` (String), `Is Comment` (Bool).

Drop a single inline robot-program line (KRL, RAPID, etc.) into the program flow.
This generic action is intentionally kept so users can emit arbitrary controller code beyond the built-in action nodes.
...
### Play / Stop simulation loop
Use **Play** in the **Task & Simulation** box to loop the simulation continuously. Use **Stop** to end the loop. Playback requires a loaded task.

### Export generated robot program
After **Load Task**, use **Export Program** to save the generated program text to disk.

- Default extension is selected by driver (`.src` for KUKA, `.mod` for ABB, `.script` for UR, else `.txt`).
- If a **Custom Header** text is configured, it is kept at the top of the exported file.
- Any `PRC Function Definition` content is appended as `DEF UP<ID>() ... END` blocks after the main program.

### Import `.src` program
Use **Import .src Program** (enabled after setup) to parse a KUKA `.src` file and load it directly into `PRC_Program`.

- Parsed commands: `PTP {...}` and `LIN {...}` blocks.
- For PTP blocks with `A1..A6`, the importer creates AXIS points.
- For cartesian PTP/LIN blocks (`X,Y,Z,A,B,C`), the importer creates PTP/LIN waypoints.
- After import, the add-on auto-builds and auto-loads the task.

### Status message utilities
The **Status** box now exposes the live status field and **Copy Status** to copy the full message to clipboard.
...
## 10. Program IO Quick Recipes

### A. Import legacy KUKA code and simulate
1. Setup Robot and connect.
2. Click **Import .src Program** and choose a KUKA `.src` file.
3. The add-on imports motion blocks and auto-loads the task.
4. Use **Play** or the **Simulation** slider to inspect the path.

### B. Build with actions and export
1. Build your GN task with motion groups and action nodes.
2. Optional: add `PRC Function Definition` for reusable/interrupt subprogram bodies.
3. Click **Load Task**.
4. Click **Export Program** and save with the suggested extension.

## 11. Putting It All Together — A Realistic Recipe
...
To reach a real 1:1 simulation workflow, the next milestone is full execution semantics for motion and non-motion actions.

1. Complete workflow simulation engine
- Simulate mixed sequences in execution order: motion, pause/wait, I/O writes, I/O waits, interrupt declaration/control.
- Keep timeline continuity so pauses and waits affect total task duration exactly as on controller logic.

2. Pause and wait parity
- Implement deterministic behavior for `WAIT SEC` and `WAIT FOR $IN[...]` in simulation playback.
- Add configurable virtual input signals so `WAIT FOR` can be tested without hardware.

3. I/O state model
- Add a simulated I/O panel/state store (inputs and outputs with timestamped changes).
- Reflect `$OUT[...]` actions in the simulated state and record transitions for debugging.

4. Interrupt runtime model
- Simulate `INTERRUPT DECL`, `INTERRUPT ON`, and `INTERRUPT OFF` lifecycle.
- Execute linked `UP<ID>` subprogram blocks when interrupt conditions are met.
- Support interrupt preemption/resume behavior so the active motion path and action stream match controller intent.

5. Validation for 1:1 confidence
- Add regression scenarios with known expected traces (timeline, I/O transitions, interrupt hits, resumed path index).
- Define pass criteria: same ordered events, same trigger points, and bounded timing error vs expected reference.
- Document controller-specific limits/assumptions where exact parity is not possible.

6. User workflow polish
- Add a simulation event log in UI (pause start/end, I/O writes, wait unblock, interrupt trigger, subprogram enter/exit).
- Add export of the simulation trace (CSV/JSON) for offline comparison and repeatability.
